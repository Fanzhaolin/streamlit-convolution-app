...
def evaluate_function(func_str, t):
    ...
    try:
        ...
        # 在 Streamlit 中使用 st.error 代替 messagebox.showerror
...
def main_convolution_app():
    st.set_page_config(layout="wide") # 使用宽布局
    ...
    t_shift = st.slider(
        ...
        value=conv_t_start, # 默认从卷积的起始时间开始
        step=dt,
        format='t = %.2f'
    )
    
    # 4. 初始化 Matplotlib 图表
    # 保持 fig size 不变，但 Streamlit 会在网页上拉伸
    fig, (ax0_1, ax0_2, ax1, ax2) = plt.subplots(
        ...
        gridspec_kw={
            'hspace': 0.6,
            'height_ratios': [1, 1, 1.5, 1.5] # 增大下面两个图
        }
    )
    ...
    ax1.set_title('卷积过程: $f_1(\\tau)$ 和 $f_2(t-\\tau)$', fontsize=10)
    ax1.set_xlabel('$\\tau$')
    ax1.set_ylabel('幅度')
    ...
    if idx_max > 0:
        ...
        # 滑块在起始时间之前，只绘制红点在起始点
        ax2.plot([conv_t_start], [0.0], 'ro', markersize=8, label='当前积分结果')

    ax2.legend(loc='upper right')
    
    # 5. 在 Streamlit 页面上显示图表
    st.pyplot(fig)
